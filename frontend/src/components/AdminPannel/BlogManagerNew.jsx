...
const BlogManager = () => {
  ...
  // Fix the input change handler - remove useCallback to prevent stale closures
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (currentBlog) {
        const updatedBlog = await blogService.updateBlog(currentBlog.id, blogData);
  ...
  // BlogList component remains the same
  const BlogList = () => (
          ...
          <>
                  ...
                  {filteredBlogs.length === 0 ? (
                    ...
                    filteredBlogs.map(blog => (
                      <tr key={blog.id}>
                            ...
                            <div>
                              ...
                              <div className="mt-1">
                                {blog.tags.slice(0, 3).map(tag => (
                                  <span key={tag} className="badge bg-light text-dark me-1" style={{ fontSize: '0.7rem' }}>
                                    {tag}
                                  </span>
                                ))}
                                {blog.tags.length > 3 && (
                        ...
                        <td>
                          <div className="fw-medium">{blog.views.toLocaleString()}</div>
                        </td>
                        <td>
                          <div className="small">
                            {blogService.formatDate(blog.updatedAt)}
                          </div>
                        </td>
                        <td>
            ...
            {filteredBlogs.length > 0 && (
              <div className="d-flex justify-content-between align-items-center mt-3">
                ...
                <div className="text-muted small">
                  Total Views: {blogs.reduce((sum, blog) => sum + blog.views, 0).toLocaleString()}
                </div>
              </div>
            )}
          </>
...
// Move BlogForm outside as a separate component to prevent re-creation
